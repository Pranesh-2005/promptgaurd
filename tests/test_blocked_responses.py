...
from promptgaurd.providers import OpenAIAdapter, AnthropicAdapter, GeminiAdapter
...
class TestOpenAIMock:
    def test_attack_returns_mock_not_exception(self):
        fake = FakeOpenAI()
        adapter = OpenAIAdapter(fake, gaudrial=Gaudrial(policy="strict"))
        ...
        assert r.id.startswith("promptgaurd-blocked-")
...
class TestAnthropicMock:
    def test_attack_returns_mock(self):
        fake = FakeAnthropic()
        adapter = AnthropicAdapter(fake, gaudrial=Gaudrial(policy="strict"))
...
class TestGeminiMock:
    def test_attack_returns_mock(self):
        fake = FakeGemini()
        adapter = GeminiAdapter(fake, gaudrial=Gaudrial(policy="strict"))
...
class TestTraceability:
    def test_block_logged_with_action_and_matching_prompt_id(self):
        entries = []
        g = Gaudrial(policy="strict", log_sink=entries.append)
        fake = FakeOpenAI()
        adapter = OpenAIAdapter(fake, gaudrial=g)
        r = adapter.chat.completions.create(
            model="gpt-4", messages=[{"role": "user", "content": ATTACK}]
        )
        ...
        assert r.id == f"promptgaurd-blocked-{prompt_id}"
    ...
    def test_custom_block_message(self):
        g = Gaudrial(policy="strict", block_message="Denied: {reason}")
